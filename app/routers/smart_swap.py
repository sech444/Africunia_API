...
def xrp_transaction(Network: Ripple_network,response: Response,token: str = Depends(token_auth_scheme),private_key=Form(...),account_to=Form(...),value_to_send=Form(...),destination_tag: Optional[int] = Form(None, description="Please comfirm if the recciving address reuires a MEMO/Tag."),):
    ...
    try:
        ...
        xrp_wallet.sequence = get_next_valid_seq_number(
            xrp_wallet.classic_address, client
        )
...
def transaction_receipt(
    tx_hash: str = Form(...), webhook_url: str = Form(...)
) -> dict():
    JSON_RPC_URL = "https://xrplcluster.com"
    client = JsonRpcClient(JSON_RPC_URL)
    try:  # print(json.dumps(tx_hash.result, indent=4, sort_keys=True))
        data = xrpl.transaction.get_transaction_from_hash(
...
async def send_tron(
    ...
    token: str = Depends(token_auth_scheme),
    sender_address=Form(...),
    recipient_address=Form(...),
    account_to_send=Form(...),
    private_key=Form(...),
):
    """A valid access token is required to access this route"""
    ...
    try:
        ...
        txn = (
            client.transfer(
                str(WALLET_ADDRESS), str(recipient_address), int(account_to_send)
            )
...
def transation_detail(
    transaction_hash: str = Form(...), webhook_url: str = Form(...)
) -> dict():
    client = Tron()
    try:
...
def create_wallet_on_tron_network():
    ...
    return {
        "Wallet address": wallet["base58check_address"],
...
def create_wallet_on_xrp_network():
    ...
    return {"classic_address": my_wallet.classic_address, "seed": my_wallet.seed}
...
async def get_afcash_exl20(
    ...
    if not adr_verify:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Invaild exl20_afcash wallet"
        )
...
async def fund_me(
    ...
    webhook_url: Optional[str] = Form(
        None,
        description="returns the tx hash to the URL that you will provide (that is an HTTP request)",
    ),
):
    w3 = Web3(
        Web3.HTTPProvider(

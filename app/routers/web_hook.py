...
w3 = Web3(
    Web3.HTTPProvider("https://mainnet.infura.io/v3/bde4e3babba54474844b65de59d0a039")
)


@router.post("/api/v1/api/eth_webhook", tags=["WebHook"])
def eth_transaction_receipt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Not a valid eth tx_hash"
        )
...
def transaction_receipt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Not a valid exl tx_hash"
        )
...
def transaction_receipt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Not a valid bnb tx_hash"
        )
...
def btc_webhook(btc_txhash: str = Form(...), webhook_url: str = Form(...)):
    try:
        ...
        data = {"acc": "transaction", "details": tx_hash}
        r = requests.post(webhook_url, data=json.dumps(data))

        return {"btc_txhash": tx_hash}
    except:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Not a valid btc tx_hash"
        )
...
def dash_webhook(dash_txhash: str = Form(...), webhook_url: str = Form(...)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Not a valid dash tx_hash"
        )
...
def ltc_webhook(ltc_txhash: str = Form(...), webhook_url: str = Form(...)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Not a valid ltc tx_hash"
        )

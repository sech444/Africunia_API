...
async def get_network(Network: BNB_network, account_from: str = Form(...), value_to_send: float = Form(...), account_to: str = Form(...), private_key: str = Form(...)):
    ...
    if len(TokenA) == 25:
        ...
        # print(result)
        # return bsc
    bsc_ = "https://bsc-dataseed.binance.org/"
    bsc_w3 = Web3(Web3.HTTPProvider(bsc_))
...
async def get_swap(swap_tokenA: Coin_symbol, swap_tokenB: Coin_symbol, account_to_swap: float = Form(...), account_from: str = Form(...), account_to: str = Form(...), private_key: str = Form(...)):
    ...
    try:
        ...
        input_abi = requests.get(
            f"{url}?apikey={apikey}&module=contract&action=getabi&address={input_address}").json()["result"]
        pswap_abi = requests.get(
            f"{url}?apikey={apikey}&module=contract&action=getabi&address={pswap_router_address}").json()["result"]
        ...
        signed = web3.eth.account.sign_transaction(
            pswap_txn, private_key=privatekey)
        tx = web3.eth.send_raw_transaction(signed.rawTransaction)

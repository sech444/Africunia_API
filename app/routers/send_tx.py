...
def busd_transaction(response: Response, token: str = Depends(token_auth_scheme), account_from: str = Form(...), account_to: str = Form(...), value_to_send: float = Form(...), Private_key: str = Form(...)):
    ...
    bsc = "https://bsc-dataseed.binance.org/"
    bsc_w3 = Web3(Web3.HTTPProvider(bsc))
    busd_addr = bsc_w3.toChecksumAddress(
    ...
    # print("sending567")
    #value_2 = int(float(value))
    trans = usdt_bep.functions.balanceOf(
        bsc_w3.toChecksumAddress(account_1)).call()
    _bal2_ = bsc_w3.fromWei(trans, 'ether')
    # print(_bal2_)
    if float(value) > _bal2_:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def busd_bals(response: Response, token: str = Depends(token_auth_scheme), wallet_id: str = Form(...)):
    ...
    try:
        _trans = usdt_bep.functions.balanceOf(
            bsc_w3.toChecksumAddress(wallet_id)).call()
        _bal2_ = bsc_w3.fromWei(_trans, 'ether')
        return {"balance": _bal2_}
    except:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def _prepare_tx_bch(response: Response, token: str = Depends(token_auth_scheme), priv_key=Form(...), addr_from=Form(...), addr_to=Form(...), value=Form(...), fee=Form(...), change_addr=Form(...), segwit=True):  # create unsigned txobj with change output
    ...
    try:
        ...
        tx = c.preparesignedtx(self=Bitcoin, privkey=privkey,
                               to=addr_to, value=int(value), addr=(addr_from))
        data = c.pushtx(tx)
        print(data)
        return{"data": data}
    except ValueError as e:

...
w3 = Web3(
    Web3.HTTPProvider("https://goerli.infura.io/v3/2b4e6cbc9f444bed94a86178238f8cad")
)
...
class EventScanner:
    ...
    For example, you can get all depositDones from all tokens in the same scan.
    ...
    def estimate_next_chunk_size(self, current_chuck_size: int, event_found_count: int):
        ...
        * Do not overload node serving JSON-RPC API by asking data for too many events at a time
        ...
        When any depositDones are encountered, we are back to scanning only a few blocks at a time.
    ...
    def scan(
        ...
        while current_block <= end_block:
            ...
            logger.debug(
                "Scanning token depositDones for blocks: %d - %d, chunk size %d, last chunk scan took %f, last logs found %d",
                current_block,
                estimated_end_block,
                chunk_size,
                last_scan_duration,
...
if __name__ == "__main__":
    # Simple demo that scans all the token depositDones of RCC token (11k).
    ...
    # RCC has around 11k depositDone events
    # https://etherscan.io/token/0x9b6443b0fb9c241a7fdac375595cea13e6b7807a
    foun_me_address = Web3.toChecksumAddress(
        "0x29465Aa45c1c137822878c4ee3107CF7B7A7DEF2"
    )

    # Reduced ERC-20 ABI, only depositDone event
    ABI = """[
	{
		"inputs": [],
    ...
    class JSONifiedState(EventScannerState):
        ...
        def process_event(
            ...
            """Record a ERC-20 depositDone in our database."""
            # Events are keyed by their transaction hash and log index
            # One transaction may contain multiple events
            # and each one of those gets their own log index

            # event_name = event.event # "depositDone"
            log_index = event.logIndex  # Log index within the block
            # transaction_index = event.transactionIndex  # Transaction index within the block
            txhash = event.transactionHash.hex()  # Transaction hash
            block_number = event.blockNumber

            # Convert ERC-20 depositDone event to our internal format
            args = event["args"]
            depositDone = {
                "from": args["from"],
                "to": args.to,
            ...
            if txhash not in block:
                # We have not yet recorded any depositDones in this transaction
                # (One transaction may contain multiple events if executed by a smart contract).
                # Create a tx entry that contains all events by a log index
                self.state["blocks"][block_number][txhash] = {}

            # Record ERC-20 depositDone in our database
            self.state["blocks"][block_number][txhash][log_index] = depositDone
    ...
    def run():
        ...
        scanner = EventScanner(
            ...
            contract=ERC20,
            state=state,
            events=[ERC20.events.depositDone],
            filters={"address": foun_me_address},
        ...
        print(
            f"Scanned total {len(result)} depositDone events, in {duration} seconds, total {total_chunks_scanned} chunk scans performed"
        )
